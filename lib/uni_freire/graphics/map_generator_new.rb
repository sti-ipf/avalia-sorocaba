module UniFreire
  module Graphics
    class MapGenerator
      ...
      def self.generate(params={})
        params = {:header_height => "30px", :file_name => "mapa", :with_colors => true,
                  :institution_is_legend => true, :has_students=>true}.merge(params)
        build_html(params[:data], params[:numbers], params[:institutions],
                   params[:columns_size], params[:header_height],
                   params[:with_colors], params[:institution_is_legend],params[:has_students])
    ...
    private
      ...
      def self.build_html(data, numbers, institutions, columns_size, height, with_colors,
        institution_is_legend, has_students)
...
HEREDOC
        ...
        @funcs << "Educandos" if has_students
        max_count_to_break_page = 160 / @funcs.size
        break_page_count = 0
        institutions.each do |institution|
          break_page_count += 1
          [first_table, second_table].each {|s| s << "<tr> <td rowspan = \"#{(@funcs.count+1)}\"> #{institution[0]} </td>"}
          @funcs.each do |f|
          ...
          if break_page_count == max_count_to_break_page
        ...
        if break_page_count < (max_count_to_break_page/2) || institutions.count <= max_count_to_break_page
      ...
      def self.add_data_in_table(data, institution, f, number, table, with_colors)
        number_filled = false
        begin
          value = data[institution[2]][f][number]
          #puts "#{number}: #{value}"
          if !value.nil?
